...
Khi chạy với `NODE_ENV=production`, backend dùng **SQLite session store** (file `data/sessions.sqlite`) thay cho MemoryStore; cảnh báo "MemoryStore is not designed for a production environment" sẽ không còn.
...
| **Cảnh báo: connect.session() MemoryStore is not designed for production** | Session đang dùng bộ nhớ, dễ rò rỉ và không scale | Khi chạy với **NODE_ENV=production**, backend tự dùng **SQLite session store** (file `backend/data/sessions.sqlite`). Nếu log báo **Cannot find module 'better-sqlite3-session-store'**: chạy `cd backend && npm install` rồi `pm2 restart homestay-api`. Đảm bảo khởi động bằng `NODE_ENV=production pm2 start server.js --name homestay-api`. |
