...
| **Cảnh báo: connect.session() MemoryStore is not designed for production** | Session đang dùng bộ nhớ, dễ rò rỉ và không scale | Khi chạy với **NODE_ENV=production**, backend tự dùng **SQLite session store** (file `backend/data/sessions.sqlite`). Đảm bảo đã `npm install` trong backend (có `better-sqlite3-session-store`) và khởi động bằng `NODE_ENV=production pm2 start server.js --name homestay-api`. Cảnh báo sẽ biến mất. |
